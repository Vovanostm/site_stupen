...
function scroll_left(area)
{
  	...
  	if (area = "b_n")
  	{
   		...
   		$area.scrollTo('section:eq('+x+')', 900, { axis:'x' });
	}
   	else
   	{
   		s_proj=s_proj<=1?0:s_proj-1;
   		$area.scrollTo('section:eq('+s_proj+')', 900, { axis:'x' });
...
now_news=now_news-1;
  ...
  success: function(data){ 
   ...
   if (area = "b_n")
   {
    ...
    x=x>=$("section.news").length-1?$("section.news").length-1:x+1;
    $area.scrollTo('section:eq('+x+')', 900, { axis:'x' });
   }
  else
  {
...
{
    $.scrollTo(block_id, 900, {axis:'y', margin:'false' });
...
{
  if (($('#newss')[0].getBoundingClientRect().top)<120)
  {
    console.log("dynamic");
